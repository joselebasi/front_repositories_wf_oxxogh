...
export default function RepositoryClosedPullRequest() {
  ...
  useEffect(() => {
    const fetchRepositories = async () => {
      const { data, error } =
        await actions.bo_closed_pull_requests_data.getAllBoClosedPullRequestsData();
      if (error) {
        console.error(error);
        return;
      }
    ...
    const fetchLastUpdate = async () => {
      const { data, error } =
        await actions.bo_table_last_update_data.getTableLastUpdateByTableName({
          table_name: "bo_closed_pull_requests",
        });
      console.log("Fetching last update for bo_closed_pull_requests");
      if (error) {
        console.error(error);
        return;
      }
  ...
  const filteredPullRequests = repositories.filter((pr) => {
    const matchesSearch =
      ...
      pr.target_branch?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pr.name_repository?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pr.label?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pr.reviewers?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const sortedPullRequests = [...filteredPullRequests].sort((a, b) => {
    ...
    if (sortColumn === "closed_at") {
      valA = new Date(a.closed_at || 0);
      valB = new Date(b.closed_at || 0);
    } else {
  ...
  const hasResults = sortedPullRequests.length > 0;
  const firstVisibleItem = sortedPullRequests.length === 0 ? 0 : startIndex + 1;
  const lastVisibleItem = Math.min(endIndex, sortedPullRequests.length);
  ...
  const downloadCSV = () => {
    ...
    const csvContent = [
      ...
      ...sortedPullRequests.map((pr) => {
        return [
          ...
          `"${pr.project || "N/A"}"`,
          `"${pr.title || "N/A"}"`,
          pr.source_branch || "N/A",
          pr.target_branch || "N/A",
          pr.status || "N/A",
          pr.label || "N/A",
          pr.closed_at
            ? new Date(pr.closed_at).toISOString().split("T")[0]
            : "N/A",
          `"${pr.reviewers || "N/A"}"`,
        ].join(",");
      }),
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "closed_pull_requests.csv");
    ...
    <div className="w-full space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white dark:bg-[#303030] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
        <div className="flex flex-col md:flex-row items-center gap-4 flex-1">
          <div className="relative w-full md:w-64">
            ...
            <input
              type="text"
              placeholder="Buscar PR, rama, proyecto o label..."
              value={searchTerm}
              onChange={(e) => {
        ...
        <div className="text-sm text-gray-600 dark:text-gray-300 font-semibold">
          ...
          {lastUpdate ? formatDate(lastUpdate) : "Cargando..."}
        </div>
      ...
      <div className="overflow-x-auto rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <table className="w-full border-collapse bg-white dark:bg-[#303030]">
          <thead>
            <tr
              className="bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-500 text-gray-900"
              style={{
                background:
                  "linear-gradient(to right, #ffc627, #ffb627, #ffc627)",
              }}
            >
              <th
              ...
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider w-[18%]">
                Proyecto
              </th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider w-[24%]">
                Titulo
              </th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {currentPullRequests.length === 0 ? (
              <tr>
                ...
                >
                  No se encontraron Pull Requests cerrados.
                </td>
              </tr>
            ) : (
              currentPullRequests.map((pr) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm font-bold whitespace-normal break-words w-[24%]">
                    {pr.url ? (
                      <a
                        href={pr.url}
                        target="_blank"
                ...
                <button
                  ...
                  style={
                    currentPage === pageNum
                      ? {
                          background:
                            "linear-gradient(to right, #ffc627, #ffb627)",
                        }
                      : {}
                  }
                >
